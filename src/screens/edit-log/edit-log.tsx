...
import { useGetSessionDetails, useUpdateWorkoutSession } from "@/hooks/useWorkoutLogging"
import { Route } from "@/routes/edit-log/$sessionId"
import { useNavigate } from "@tanstack/react-router"
import EditLogHeader from "./edit-log-header"
...
const EditLog = () => {
    ...
    const { updateWorkoutSession } = useUpdateWorkoutSession(Number(sessionId))

    useEffect(() => {
        if (sessionDetails) {
            const sets = sessionDetails.sets.map((set) => ({
                weight: set.weight.toString(),
                reps: set.reps.toString()
            }))
            setSets(sets)
            setExerciseId(sessionDetails.exerciseId)
        }
    }, [sessionDetails])
    ...
    const handleSaveWorkout = async () => {
        try {
            ...
            await updateWorkoutSession(workoutSets)
            ...
            <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
                <div className="p-4 flex items-center justify-center min-h-[400px]">
                    <div className="text-center">
                        <p className="text-muted-foreground mb-4">Exercise log not found</p>
                        <Button onClick={handleBack}>Back to Logs</Button>
                    </div>
                </div>
            </div>
        )
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
            ...
            <h2 className="px-4 pt-2 block text-base font-semibold text-slate-900 dark:text-white tracking-tight mb-1 truncate">
                {capitalizeFirst(sessionDetails.exerciseName)}
            </h2>
