...
class ExerciseService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `✅ Exercise Service ready with ${this.exercises.length} exercises`
      );
    } catch (error) {
  ...
  private ensureInitialized(): void {
    if (!this.initialized || !this.fuse) {
      throw new Error(
        "Exercise Service not initialized. Call initialize() first."
      );
    }
  }
    ...
    const dumbbellExercises = this.exercises
      .filter(
        (exercise) =>
          ...
          !bodyWeightExercises.some(
            (bw) => bw.exerciseId === exercise.exerciseId
          )
      )
  ...
  async getExerciseStats(): Promise<ExerciseStats> {
    ...
    this.exercises.forEach((exercise) => {
      ...
      // Muscles
      [...exercise.targetMuscles, ...exercise.secondaryMuscles].forEach(
        (muscle) => {
  ...
  async getFilterOptions(): Promise<{
    ...
    const bodyParts = [
      ...new Set(this.exercises.flatMap((e) => e.bodyParts)),
    ].sort();
    const muscles = [
      ...new Set(
        this.exercises.flatMap((e) => [
          ...e.targetMuscles,
          ...e.secondaryMuscles,
        ])
      ),
  ...
  private applyFilters(
    ...
    if (options.bodyPart) {
      filtered = filtered.filter((exercise) =>
        exercise.bodyParts.some(
          (part) => part.toLowerCase() === options.bodyPart!.toLowerCase()
        )
      );
    }
    ...
    if (options.equipment) {
      filtered = filtered.filter((exercise) =>
        exercise.equipments.some(
          (eq) => eq.toLowerCase() === options.equipment!.toLowerCase()
        )
      );
    }
    ...
    if (options.muscle) {
      filtered = filtered.filter((exercise) =>
        [...exercise.targetMuscles, ...exercise.secondaryMuscles].some(
          (muscle) => muscle.toLowerCase() === options.muscle!.toLowerCase()
        )
      );
    }
  ...
  private categorizeExercise(
    ...
    if (
      exercise.equipments.some((eq) =>
        ["yoga mat", "stability ball", "foam roller", "roller"].includes(
          eq.toLowerCase()
        )
      )
    ) {
